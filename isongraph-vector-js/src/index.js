 ...
 *
 * This replaces a scheme that emitted the low 16 bits of an LCG. The low bits
 * of an LCG evolve independently of the high bits, so every vector was
...
export class MockEncoder {
  static FNV_OFFSET_BASIS = 0x811c9dc5;
  ...
  encode(text) {
    ...
    // rather than using TextEncoder so this behaves identically in Node and
    // the browser with no global to depend on.
...
export class EmbeddingStore {
  ...
  addBatch(entries) {
    ...
    entries.forEach((entry, i) => {
      const vector = vectorFor.has(i) ? vectorFor.get(i) : entry[2];
...
export class SemanticGraph extends ISONGraph {
  constructor(options = {}) {
    ...
    this._embeddingStore = new EmbeddingStore(this._encoder);
   ...
   *
   * Accepts either an options object as the second argument or the original
   * positional form, so existing callers keep working.
   */
  semanticMultiHop(
    query,
    relTypeOrOptions,
    ...
    for (const seed of seeds) {
      ...
      const queue = [{ nodeRef: seed.nodeRef, path: [seed.nodeRef], hop: 0 }];
...
// Re-export from ison-graph-js
export { ISONGraph, Direction } from 'ison-graph-js';

/**
 * ISONGraph Vector JavaScript Tests
 */
...
describe('EmbeddingStore', () => {
  ...
  it('should perform similarity search', () => {
    ...
    // Use negative threshold for mock encoder
    const results = store.similaritySearch('software engineer', 3, undefined, -1.0);
...
describe('MockEncoder quality and cross-language parity', () => {
  it('produces the same vectors as every other language port', () => {
    ...
    for (const [text, want] of [['hello', GOLDEN.hello], ['ison', GOLDEN.ison], ['', GOLDEN.empty]]) {
...
describe('embedding portability', () => {
  ...
  it('rejects a foreign payload', () => {
    const store = new EmbeddingStore(new MockEncoder(8));
    expect(() => store.importEmbeddings({ format: 'something-else' })).toThrow();
  });
});

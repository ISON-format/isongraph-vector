 ...
 *
 * A separate entry point (`isongraph-vector-js/sqlite`) rather than part of the
 ...
 *
 * The schema is identical to the one the Python, C#, Rust, TypeScript and C++
...
export class SqliteEmbeddingStore {
     ...
     */
    constructor(path = ':memory:', encoder, sqliteVec = false) {
        ...
        if (this._sqliteVec && this._dimension !== null)
            this._ensureVecTable(this._dimension);
    }
    _loadVecExtension() {
        let sqliteVec;
        try {
            ...
            throw new EmbeddingError('sqliteVec: true needs the sqlite-vec package: npm install sqlite-vec');
        }
        try {
            ...
            throw new EmbeddingError(`could not load the sqlite-vec extension: ${err.message}. ` +
    ...
    _ensureVecTable(dimension) {
        ...
        this._db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS ${VEC_TABLE} USING vec0(
           node_type text,
           embedding float[${dimension}] distance_metric=cosine
       )`);
        const indexed = Number(this._db.prepare(`SELECT COUNT(*) AS c FROM ${VEC_TABLE}`).get().c);
        const stored = Number(this._db.prepare('SELECT COUNT(*) AS c FROM embeddings').get().c);
        if (indexed !== stored) {
            ...
            const rows = this._db
                ...
                .all();
            const insert = this._db.prepare(`INSERT INTO ${VEC_TABLE}(rowid, node_type, embedding) VALUES (?, ?, ?)`);
            for (const row of rows)
                insert.run(BigInt(row.rid), row.node_type, row.vector);
        }
        this._vecReady = true;
    }
    ...
    static _cosineSimilarity(v1, v2) {
        let dot = 0, n1 = 0, n2 = 0;
     ...
     */
    _writeRow(nodeRef, vector, text, model, nodeIdType = 'str') {
    ...
    `).run(embId, String(nodeRef[0]), String(nodeRef[1]), nodeIdType, blob, text, model);
        if (this._sqliteVec) {
            ...
            const row = this._db
                ...
                .get(String(nodeRef[0]), String(nodeRef[1]));
    ...
    addBatch(entries) {
        ...
        if (vectors.length !== entries.length) {
            throw new EmbeddingError(`encoder returned ${vectors.length} vectors for ${entries.length} texts`);
        }
    ...
    get(nodeRef) {
        const row = this._db
            .prepare('SELECT id, node_type, node_id, vector, text FROM embeddings ' +
    ...
    remove(nodeRef) {
        if (this._sqliteVec && this._vecReady) {
            const row = this._db
                ...
                .get(String(nodeRef[0]), String(nodeRef[1]));
            if (row)
                this._db.prepare(`DELETE FROM ${VEC_TABLE} WHERE rowid = ?`).run(BigInt(row.rid));
        }
    ...
    count() {
        return Number(this._db.prepare('SELECT COUNT(*) AS c FROM embeddings').get().c);
    }
    clear() {
    ...
    _scan(queryVector, nodeType) {
        ...
        const results = [];
        for (const row of rows) {
            if (nodeType && row.node_type !== nodeType)
                continue;
            results.push({
                nodeRef: [row.node_type, row.node_id],
    ...
    _vecSearch(queryVector, topK, nodeType, threshold) {
        this._ensureVecTable(queryVector.length);
        const blob = SqliteEmbeddingStore._pack(queryVector);
        const select = `SELECT e.node_type AS node_type, e.node_id AS node_id, e.text AS text, ` +
        ...
        for (const row of rows) {
            ...
            if (score >= threshold) {
                results.push({ nodeRef: [row.node_type, row.node_id], score, text: row.text });
            }
        }
        return results;
    }
    ...
    exportEmbeddings() {
        const rows = this._db
            .prepare('SELECT node_type, node_id, node_id_type, vector, text FROM embeddings ' +
            'ORDER BY node_type, node_id')
            .all();
        const embeddings = rows.map(row => ({
            type: row.node_type,
            id: row.node_id,
            id_type: (row.node_id_type === 'int' ? 'int' : 'str'),
            text: row.text,
            vector: SqliteEmbeddingStore._unpack(row.vector),
        }));
        return {
    ...
    importEmbeddings(payload, replace = false) {
        ...
        try {
            for (const item of payload.embeddings ?? []) {
                ...
                this._writeRow([item.type, String(item.id)], item.vector, item.text ?? '', 'unknown', item.id_type ?? 'str');
                written++;
            }
            this._db.exec('COMMIT');
        }

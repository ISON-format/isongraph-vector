...
describe('SqliteEmbeddingStore', () => {
  ...
  it('overwrites rather than duplicating', () => {
    ...
    expect(store.count()).toBe(1);
    expect(store.get(['a', '1']).text).toBe('second');
    store.close();
  });
  ...
  it('batches in one transaction', () => {
    const store = open();
    const entries = Array.from({ length: 10 }, (_, i) => [['doc', String(i)], `document ${i}`]);
  ...
  it('survives close and reopen', () => {
    ...
    const probe = first.get(['person', '1']).vector;
    first.close();
    expect(existsSync(path)).toBe(true);

    const reopened = open(false, 32, path);
    expect(reopened.count()).toBe(1);
    expect(reopened.get(['person', '1']).vector).toEqual(probe);
  ...
  it('round-trips the portable payload', () => {
    ...
    expect(target.get(['account', '007']).text).toBe('Bond');
    source.close();
    target.close();
  });
});
...
describe('sqlite-vec index', () => {
  ...
  it('matches the scan exactly', () => {
    ...
    const entries = Array.from({ length: 200 }, (_, i) => [['n', String(i)], `text ${i}`]);
  ...
  it('reaches the index on update and delete', () => {
    ...
    const second = store.get(['a', '2']).vector;
  ...
  it('indexes a database written without it', () => {
    ...
    const probe = plain.get(['n', '7']).vector;
    plain.close();
